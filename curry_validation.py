"""
Stephen Curry Fantasy Basketball Model Validation

Validation focusing on 5 stats: FGM, REB, FTA, AST, 3PTA
...
def load_and_filter_data(csv_path: str, player_name: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    print(f"Found {len(player_data)} {player_name} games")

    if len(player_data) == 0:
        print(f"ERROR: No games found for {player_name}")
        return None, None
...
class SimpleBayesianModel:
    """
    Simplified Bayesian model for multiple statistics.
    ...
    def fit(self, training_data: pd.DataFrame, recency_weight: bool = False):
        """
...
def main():
    ...
    print("="*70)
    print("STEPHEN CURRY FANTASY BASKETBALL MODEL VALIDATION")
    print("="*70)
    ...
    if training_data is None or len(validation_data) == 0:
        print("\nERROR: Insufficient data. Cannot proceed.")
        return

    # Build and fit model
    model = SimpleBayesianModel(stats_to_model)
    model.fit(training_data, recency_weight=False)
    ...
    viz_path = create_visualization(simulations, validation_data, stats_to_model,
                                    player_name, 'curry_validation_viz.png')
